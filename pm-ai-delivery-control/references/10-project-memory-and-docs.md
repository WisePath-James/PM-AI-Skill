...
### Memory Boot Evidence Rule

PM AI 完成 Memory Boot 后，必须在回复中引用至少 3 个具体状态字段，证明已完成上下文加载。可引用的状态字段包括：当前阶段/迭代、Scope Baseline 版本、当前进行中 WP、最新决策/变更/风险、当前阻塞项、下一步行动。不得只写"已读取 Memory"。
